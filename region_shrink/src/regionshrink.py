...
def estimate_scales(image: np.ndarray, pagetype: Optional[PageType] = None) -> int:
    """
    ...
    if len(stats) > 1:  # stats[0] is the background
        ...
        if pagetype and pagetype in ESTIMATE_MAX:
            ...
            max(1, min(round(avg_width), round(avg_height)))
    return 25
...
def shrink(
    ...
    exclude: Optional[list[PageType]] = None
) -> PageXML:
    """
    ...
    Args:
        ...
        smoothing: moothing of the region, calculated as the factor of the average glyph size. Defaults to 1.0.
    ...
    for region in xml.regions:
        ...
        if coords is None or "points" not in coords:
            log.warning(f"Could not find valid Coords element in {region} ({pagexml})")
            continue
